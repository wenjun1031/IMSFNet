...
class SPRNet(nn.Module):
    def __init__(self,channel=192):
        ...
        self.sprm1 = SPRM(2048,channel)#统一降维成64
        self.sprm2 = SPRM(1024,channel)
        self.sprm3 = SPRM(512,channel)
        self.sprm4 = SPRM(256,channel)
        self.sprm5 = SPRM(64,channel)
        self.gfem  = GFEM(2048,channel)
    ...
    def forward(self, x):#HFAM模型只进行了一次监督
        ...
        f3_2 = F.interpolate(f3_,x2_size,mode='bilinear',align_corners=True)
        y2 = self.sprm4(x2)
        ...
        f2_1 = F.interpolate(f2_,x1_size,mode='bilinear',align_corners=True)
        y1 = self.sprm5(x1)
        f1_= torch.cat((f2_1,y1),1)
        f1_= self.reduce3(f1_)
        f1_= torch.add(f2_1,f1_)+self.se(y1)
        f1_= torch.add(y1,f1_)
        f1 = self.reduce(f1_)

        score1 = F.interpolate(f1,x_size,mode='bilinear',align_corners=True)
